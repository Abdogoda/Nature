...
const visaContainer = `<div class="credit-card-con" id="credit-card-con">
						<div class="card-container">
							<div class="front">
								<div class="image">
									<img src="assets/img/chip.png" alt="" /> <img
										src="assets/img/visa.png" alt="" />
								</div>
						...
						<div class="form">
							<div class="input-flex-boxes">
								<div class="input-box">
									<label>card number</label> <input type="text" id="card_number" name="card_number" minlength="16" length="16" maxlength="16"
										class="card-number-input"  />
								</div>
								<div class="input-box">
									<label>card holder</label> <input type="text" name="card_holder" id="card_holder" minlength="3"
							...
							<div class="input-box">
								<label for="cvv">CVV</label> <input type="text" minlength="4" length="4" maxlength="4"
									class="cvv-input"  name="card_cvv" id="card_cvv" />
							</div>
						</div>
					</div>`
...
paymentMethod.forEach(method => {
	method.addEventListener('change', function() {
		if (this.value == "pay_with_credit_card") {
...
function isValidDate(input) {
	var today = new Date();
	var userInputDate = new Date(input.value)
...
if (checkout_form) {
	...
	checkout_form.addEventListener('submit', function(event) {
		...
		document.querySelectorAll(".error").forEach(element => {
		 ...
		 if (!isMoreThanThree(city)) {
		...
		} else if (!isMoreThanThree(state)) {
			createToast("warning", "State Name Must Be More Than 3 Characters!");
			...
			if (!isValidNumber(card_number.value)) {
				createToast("warning", "Card Number Must Be A Number With Length Of 16 Characters!");
				card_number.classList.add("error");
			} else if (!isValidNumber(card_cvv.value)) {
				createToast("warning", "Card CVV Must Be A Number With Length Of 4 Characters!");
				card_cvv.classList.add("error");
			} else if (!isValidDate(expiration_date)) {
